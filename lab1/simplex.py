...
def read_problem_from_file(filename):
    ...
    Формат файла:
        ...
        constraints:
            ...
            ограничение_2
            ...
        free_variables: x_2 x_4

    Аргументы:
        filename: путь к файлу с описанием задачи

    Возвращает:
        кортеж (num_vars, constraints, objective_function, free_variables)
    """
    ...
    for line in lines:
        ...
        elif line.lower().startswith('free_variables:'):
            in_constraints = False
            # Парсинг свободных переменных
            parts = line.split(':', 1)
            if len(parts) > 1:
                free_vars_str = parts[1].strip()
                for token in free_vars_str.split():
                    if '_' in token:
                        try:
                            _, idx = token.split('_')
                            free_variables.append(int(idx))
                        except:
                            pass
    ...
    return num_vars, constraints, (objective_type, objective_expr), free_variables
...
class SimplexSolver:
    ...
    - Ограничения типов <=, >=, =
    - Свободные переменные (не обязательно неотрицательные)
    ...
    def __init__(self, num_vars, constraints, objective_function, free_variables=None, max_iterations=MAX_ITERATIONS):
        """
        ...
        Аргументы:
            ...
            free_variables: список индексов свободных переменных (например, [2, 4])
            max_iterations: максимальное количество итераций (по умолчанию 1000)
        """
        self.original_num_vars = num_vars
        self.free_variables = free_variables if free_variables else []
        ...
        # Создание маппинга для свободных переменных: x_i -> (x_i^+, x_i^-)
        # Для обычных переменных: x_i -> (x_i, None)
        self.var_mapping = {}
        current_var_index = 0
        
        for i in range(1, num_vars + 1):
            if i in self.free_variables:
                # Свободная переменная: x_i = x_i^+ - x_i^-
                self.var_mapping[i] = (current_var_index, current_var_index + 1)
                current_var_index += 2
            else:
                # Обычная неотрицательная переменная
                self.var_mapping[i] = (current_var_index, None)
                current_var_index += 1
        
        # Новое количество переменных после замены свободных
        self.num_vars = current_var_index

        # Преобразование ограничений и целевой функции для свободных переменных
        self.constraints = self._transform_constraints(constraints)
        self.objective_expression = self._transform_expression(self.objective_expression)
    ...
    def _transform_expression(self, expression):
        """
        Преобразование выражения (целевой функции или ограничения) для свободных переменных.
        
        Заменяет x_i на (x_i^+ - x_i^-) для свободных переменных.
        ...
        while i < len(tokens):
            ...
            if '_' in token:
                ...
                parts = token.split('_')
                if len(parts) == 2:
                    coeff_str = parts[0][:-1]  # убираем 'x'
                    var_idx = int(parts[1])
                    
                    # Получаем знак (смотрим на предыдущий токен)
                    sign = '+'
                    if i > 0 and tokens[i - 1] == '-':
                        sign = '-'
                        new_tokens.pop()  # Удаляем знак минус, мы его учтём
                    elif i > 0 and tokens[i - 1] == '+':
                        new_tokens.pop()  # Удаляем знак плюс
                    
                    if var_idx in self.var_mapping:
                        pos_idx, neg_idx = self.var_mapping[var_idx]
                        
                        if neg_idx is not None:
                            # Свободная переменная: заменяем на x_i^+ - x_i^-
                            if sign == '+':
                                new_tokens.extend(['+', f'{coeff_str}x_{pos_idx + 1}', '-', f'{coeff_str}x_{neg_idx + 1}'])
                            else:
                                new_tokens.extend(['-', f'{coeff_str}x_{pos_idx + 1}', '+', f'{coeff_str}x_{neg_idx + 1}'])
                        else:
                            # Обычная переменная
                            if sign == '+':
                                new_tokens.extend(['+', f'{coeff_str}x_{pos_idx + 1}'])
                            else:
                                new_tokens.extend(['-', f'{coeff_str}x_{pos_idx + 1}'])
                    i += 1
                    continue
        ...
        # Очистка лишних знаков в начале
    ...
    def _transform_constraints(self, constraints):
        """
        Преобразование всех ограничений для свободных переменных.
        
        Аргументы:
            constraints: список строк с ограничениями
            
        Возвращает:
            список преобразованных ограничений
        """
        transformed = []
        for constraint in constraints:
            # Разделяем на левую часть, оператор и правую часть
            for op in ['<=', '>=', '=']:
                if op in constraint:
                    ...
                    if len(parts) == 2:
                        ...
                        left_transformed = self._transform_expression(left)
                        
                        # Собираем обратно
                        transformed.append(f"{left_transformed} {op} {right}")
                    break
            else:
        ...
        for row_idx in range(1, len(self.constraints) + 1):
            ...
            for part_idx in range(len(constraint_parts)):
                ...
                if '_' in part:
                    ...
                    if part_idx > 0 and constraint_parts[part_idx - 1] == '-':
    ...
    def _update_objective_row(self):
        ...
        for part_idx in range(len(objective_parts)):
            ...
            if '_' in part:
                ...
                if part_idx > 0 and objective_parts[part_idx - 1] == '-':
    ...
    def _extract_solution(self):
        ...
        Для свободных переменных вычисляет x_i = x_i^+ - x_i^-.

        Возвращает:
            словарь вида {'x_1': значение, 'x_2': значение, ...} 
            (в терминах исходных переменных)
        """
        # Сначала извлекаем значения всех преобразованных переменных
        ...
        for row_idx, var_idx in enumerate(self.basic_variables[FIRST_CONSTRAINT_ROW:], start=FIRST_CONSTRAINT_ROW):
            # Учитываем только переменные задачи (не slack/artificial)
            if var_idx < self.num_vars:
                transformed_solution[var_idx] = self.coefficient_matrix[row_idx][RHS_COLUMN]

        # Небазисные переменные равны нулю
        for var_idx in range(self.num_vars):
            if var_idx not in self.basic_variables[FIRST_CONSTRAINT_ROW:]:
                transformed_solution[var_idx] = Fraction("0/1")

        # Теперь восстанавливаем исходные переменные
        solution = {}
        
        for original_idx in range(1, self.original_num_vars + 1):
            pos_idx, neg_idx = self.var_mapping[original_idx]
            
            if neg_idx is not None:
                # Свободная переменная: x_i = x_i^+ - x_i^-
                x_plus = transformed_solution.get(pos_idx, Fraction("0/1"))
                x_minus = transformed_solution.get(neg_idx, Fraction("0/1"))
                solution[f'x_{original_idx}'] = x_plus - x_minus
            else:
                # Обычная переменная
                solution[f'x_{original_idx}'] = transformed_solution.get(pos_idx, Fraction("0/1"))
